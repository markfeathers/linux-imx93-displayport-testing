...
int regmap_read_debug(struct regmap *map, uint16_t reg, uint32_t *val)
{
    int ret = regmap_read(map, reg, val);
#ifdef DEBUG
    if (ret == 0) {
        printk(KERN_INFO "DISPLAYPORT: Read  - Address: 0x%04X, Data: 0x%08X\n", reg, *val);
    } else {
        printk(KERN_INFO "DISPLAYPORT: Failed to read from Address: 0x%04X\n", reg);
    }
#endif
    return ret;
}

int regmap_write_debug(struct regmap *map, uint16_t reg, uint32_t val)
{
    int ret = regmap_write(map, reg, val);
#ifdef DEBUG
    if (ret == 0) {
        printk(KERN_INFO "DISPLAYPORT: Write - Address: 0x%04X, Data: 0x%08X\n", reg, val);
    } else {
        printk(KERN_INFO "DISPLAYPORT: Failed to write to Address: 0x%04X\n", reg);
    }
#endif
    return ret;
}
...
#ifdef DEBUG
    printk(KERN_INFO "DISPLAYPORT: polling  - Address: 0x%04X, mask: 0x%08X, data: 0x%08X\n", addr, cond_mask, cond_value);
#endif
			       ...
			       struct drm_dp_aux_msg *msg)
{
	...
	/* Store address */
	ret = regmap_write_debug(tc->regmap, DP0_AUXADDR, msg->address);
	if (ret)
		return ret;
	/* Start transfer */
	ret = regmap_write_debug(tc->regmap, DP0_AUXCFG0, tc_auxcfg0(msg, size));
	if (ret)
		return ret;
	...
	ret = regmap_read_debug(tc->regmap, DP0_AUXSTATUS, &auxstatus);
	if (ret)
		return ret;
...
static int tc_pllupdate(struct tc_data *tc, unsigned int pllctrl)
{
	...
	ret = regmap_write_debug(tc->regmap, pllctrl, PLLUPDATE | PLLEN);
	if (ret)
		return ret;
...
static int tc_pxl_pll_en(struct tc_data *tc, u32 refclk, u32 pixelclock)
{
	...
	ret = regmap_write_debug(tc->regmap, PXL_PLLCTRL, PLLBYP | PLLEN);
	if (ret)
		return ret;

	ret = regmap_write_debug(tc->regmap, PXL_PLLPARAM, pxl_pllparam);
	if (ret)
		return ret;
...
static int tc_pxl_pll_dis(struct tc_data *tc)
{
	...
	return regmap_write_debug(tc->regmap, PXL_PLLCTRL, PLLBYP);
}
...
static int tc_stream_clock_calc(struct tc_data *tc)
{
	 ...
	 *
	 */
	return regmap_write_debug(tc->regmap, DP0_VIDMNGEN1, 32768);
}
...
static int tc_set_syspllparam(struct tc_data *tc)
{
	...
	return regmap_write_debug(tc->regmap, SYS_PLLPARAM, pllparam);
}
...
static int tc_aux_link_setup(struct tc_data *tc)
{
	...
	ret = regmap_write_debug(tc->regmap, DP_PHY_CTRL,
	...
	ret = regmap_write_debug(tc->regmap, DP0_AUXCFG1, dp0_auxcfg1);
	if (ret)
		goto err;
				    ...
				    const struct drm_display_mode *mode)
{
	 ...
	 * sync signals
	 */
	ret = regmap_write_debug(tc->regmap, VPCTRL0,
			   FIELD_PREP(VSDELAY, right_margin + 10) |
			   OPXLFMT_RGB888 | FRMSYNC_ENABLED | MSF_DISABLED);
	if (ret)
		return ret;

	ret = regmap_write_debug(tc->regmap, HTIM01,
			   FIELD_PREP(HBPR, ALIGN(left_margin, 2)) |
			   FIELD_PREP(HPW, ALIGN(hsync_len, 2)));
	if (ret)
		return ret;

	ret = regmap_write_debug(tc->regmap, HTIM02,
			   FIELD_PREP(HDISPR, ALIGN(mode->hdisplay, 2)) |
			   FIELD_PREP(HFPR, ALIGN(right_margin, 2)));
	if (ret)
		return ret;

	ret = regmap_write_debug(tc->regmap, VTIM01,
			   FIELD_PREP(VBPR, upper_margin) |
			   FIELD_PREP(VSPR, vsync_len));
	if (ret)
		return ret;

	ret = regmap_write_debug(tc->regmap, VTIM02,
			   FIELD_PREP(VFPR, lower_margin) |
			   FIELD_PREP(VDISPR, mode->vdisplay));
	if (ret)
		return ret;

	ret = regmap_write_debug(tc->regmap, VFUEN0, VFUEN); /* update settings */
	if (ret)
		return ret;

	/* Test pattern settings */
	ret = regmap_write_debug(tc->regmap, TSTCTL,
				 ...
				 const struct drm_display_mode *mode)
{
	...
	return regmap_write_debug(tc->regmap, POCTRL, value);
}
				 ...
				 const struct drm_display_mode *mode)
{
	...
	ret = regmap_write_debug(tc->regmap, DP0_VIDSYNCDELAY,
		 FIELD_PREP(THRESH_DLY, max_tu_symbol) |
		 FIELD_PREP(VID_SYNC_DLY, vid_sync_dly));

	ret = regmap_write_debug(tc->regmap, DP0_TOTALVAL,
			   FIELD_PREP(H_TOTAL, mode->htotal) |
			   FIELD_PREP(V_TOTAL, mode->vtotal));
	if (ret)
		return ret;

	ret = regmap_write_debug(tc->regmap, DP0_STARTVAL,
			   FIELD_PREP(H_START, left_margin + hsync_len) |
			   FIELD_PREP(V_START, upper_margin + vsync_len));
	if (ret)
		return ret;

	ret = regmap_write_debug(tc->regmap, DP0_ACTIVEVAL,
	...
	ret = regmap_write_debug(tc->regmap, DP0_SYNCVAL, dp0_syncval);
	if (ret)
		return ret;
	...
	ret = regmap_write_debug(tc->regmap, DPIPXLFMT, dpipxlfmt);
	if (ret)
		return ret;

	ret = regmap_write_debug(tc->regmap, DP0_MISC,
...
static int tc_wait_link_training(struct tc_data *tc)
{
	...
	ret = regmap_read_debug(tc->regmap, DP0_LTSTAT, &value);
	if (ret)
		return ret;
...
static int tc_main_link_enable(struct tc_data *tc)
{
	...
	ret = regmap_read_debug(tc->regmap, DP0CTL, &value);
	if (ret)
		return ret;

	if (WARN_ON(value & DP_EN)) {
		ret = regmap_write_debug(tc->regmap, DP0CTL, 0);
		if (ret)
			return ret;
	}

	ret = regmap_write_debug(tc->regmap, DP0_SRCCTRL, tc_srcctrl(tc));
	if (ret)
		return ret;
	/* SSCG and BW27 on DP1 must be set to the same as on DP0 */
	ret = regmap_write_debug(tc->regmap, DP1_SRCCTRL,
	...
	ret = regmap_write_debug(tc->regmap, DP_PHY_CTRL, dp_phy_ctrl);
	if (ret)
		return ret;
	...
	ret = regmap_write_debug(tc->regmap, DP0_SNKLTCTRL,
			   DP_LINK_SCRAMBLING_DISABLE |
			   DP_TRAINING_PATTERN_1);
	if (ret)
		return ret;

	ret = regmap_write_debug(tc->regmap, DP0_LTLOOPCTRL,
	...
	ret = regmap_write_debug(tc->regmap, DP0_SRCCTRL,
	...
	ret = regmap_write_debug(tc->regmap, DP0CTL,
			   (drm_dp_enhanced_frame_cap(tc->link.dpcd) ?
				EF_EN : 0) | DP_EN);
	if (ret)
		return ret;
...
static int tc_main_link_disable(struct tc_data *tc)
{
	...
	ret = regmap_write_debug(tc->regmap, DP0_SRCCTRL, 0);
	if (ret)
		return ret;
...
static int tc_dsi_rx_enable(struct tc_data *tc)
{
	...
	regmap_write_debug(tc->regmap, PPI_D0S_CLRSIPOCOUNT, 5);
	regmap_write_debug(tc->regmap, PPI_D1S_CLRSIPOCOUNT, 5);
	regmap_write_debug(tc->regmap, PPI_D2S_CLRSIPOCOUNT, 5);
	regmap_write_debug(tc->regmap, PPI_D3S_CLRSIPOCOUNT, 5);
	regmap_write_debug(tc->regmap, PPI_D0S_ATMR, 0);
	regmap_write_debug(tc->regmap, PPI_D1S_ATMR, 0);
	regmap_write_debug(tc->regmap, PPI_TX_RX_TA, TTA_GET | TTA_SURE);
	regmap_write_debug(tc->regmap, PPI_LPTXTIMECNT, LPX_PERIOD);

	value = ((LANEENABLE_L0EN << tc->dsi->lanes) - LANEENABLE_L0EN) |
		LANEENABLE_CLEN;
	regmap_write_debug(tc->regmap, PPI_LANEENABLE, value);
	regmap_write_debug(tc->regmap, DSI_LANEENABLE, value);
	...
	ret = regmap_write_debug(tc->regmap, SYSCTRL, value);
	if (ret)
		return ret;

	usleep_range(120, 150);

	regmap_write_debug(tc->regmap, PPI_STARTPPI, PPI_START_FUNCTION);
	regmap_write_debug(tc->regmap, DSI_STARTDSI, DSI_RX_START);
...
static int tc_dpi_rx_enable(struct tc_data *tc)
{
	...
	return regmap_write_debug(tc->regmap, SYSCTRL, value);
}
...
static int tc_edp_stream_enable(struct tc_data *tc)
{
	...
	if (drm_dp_enhanced_frame_cap(tc->link.dpcd))
		value |= EF_EN;
	ret = regmap_write_debug(tc->regmap, DP0CTL, value);
	if (ret)
		return ret;
	/*
...
static enum drm_connector_status tc_bridge_detect(struct drm_bridge *bridge)
{
	...
	ret = regmap_read_debug(tc->regmap, GPIOI, &val);
	if (ret)
...
static const struct drm_bridge_funcs tc_dpi_bridge_funcs = {
	...
	.atomic_pre_enable = tc_dpi_bridge_atomic_enable,
...
static irqreturn_t tc_irq_handler(int irq, void *arg)
{
	...
	r = regmap_read_debug(tc->regmap, INTSTS_G, &val);
	if (r)
		return IRQ_NONE;
	...
	if (val & INT_SYSERR) {
		...
		regmap_read_debug(tc->regmap, SYSSTAT, &stat);
	...
	regmap_write_debug(tc->regmap, INTSTS_G, val);
...
static int tc_probe(struct i2c_client *client)
{
	...
	printk(KERN_INFO "HERE, DRM SHOULD BE SET UP: %s, %s, %d\n", __FILE__, __FUNCTION__, __LINE__);
	...
	if (client->irq > 0) {
		/* enable SysErr */
		regmap_write_debug(tc->regmap, INTCTL_G, INT_SYSERR);
	...
	printk(KERN_INFO "HERE: %s, %s, %d\n", __FILE__, __FUNCTION__, __LINE__);
	ret = regmap_read_debug(tc->regmap, TC_IDREG, &tc->rev);
	if (ret) {
	...
	if (tc->hpd_pin >= 0) {
		...
		/* Set LCNT to 2ms */
		regmap_write_debug(tc->regmap, lcnt_reg,
			     clk_get_rate(tc->refclk) * 2 / 1000);
		/* We need the "alternate" mode for HPD */
		regmap_write_debug(tc->regmap, GPIOM, BIT(tc->hpd_pin));
